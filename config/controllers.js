 ...
 * MainCtrl - controller
 */
function MainCtrl($scope,Cache,$http, $location) { 
     
   $scope.user = Cache.loggedInUser();
   /* this.userName = user.D_N;   
    this.ppicId = user.ppicId;*/ 
	...
	$http.get(_appUrl+'api/logout').
	  success(function(data, status, headers, config) {
		  localStorage.clear();
			...
			if(rdata.status =="S") {
				var _aName = getAppName(window.location.pathname);
				if(_aName.indexOf("polyapps") > -1) {  
				    _aName = _aName.substring(0,_aName.length - 8);
				}
				window.location.href = _appUrl+"login.jsp"; 
			} else {
     ...
     $scope.addTask = function() {
         ...
         var _dt = Util.convertDate($scope.taskrow.duedate);
             ...
             'data':{
                    ...
                    'ATTACHMENT_ID': $scope.fileAttachRow.fileId,
                    'START_DATE': _st,
                    'DUE_DATE':_dt,
...
angular
    .module('atDesk')
    .controller('MainCtrl', ['$scope','Cache','$http', '$location',MainCtrl])
